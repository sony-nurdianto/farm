...
type intercpthErrorRecorder struct {
	span  otelTrace.Span
	loggr *logs.Logger
}

func NewintercpthErrorRecorder(
	ts otelTrace.Span,
	loggr *logs.Logger,
) intercpthErrorRecorder {
	return intercpthErrorRecorder{ts, loggr}
}

func (ier intercpthErrorRecorder) Record(
	ctx context.Context,
	code codes.Code,
