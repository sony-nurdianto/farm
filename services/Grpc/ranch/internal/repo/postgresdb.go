...
import (
	...
	"github.com/sony-nurdianto/farm/services/Grpc/ranch/internal/constants/pgquery/animalquery"
...
func prepareRanchDB(
	...
	go func() {
		...
		chs := []<-chan any{
			prepareStmt(ctx, ranchDB.db, animalquery.QueryInsertAnimal, InsertAnimalStmtType),
			prepareStmt(ctx, ranchDB.db, animalquery.QueryDeleteAnimal, DeleteAnimalStmtType),

			// GetAnimal
			prepareStmt(ctx, ranchDB.db, animalquery.QueryGetAnimalASC, GetAnimalAscStmtType),
			prepareStmt(ctx, ranchDB.db, animalquery.QueryGetAnimalDESC, GetAnimalDescStmtType),
			prepareStmt(ctx, ranchDB.db, animalquery.QueryTotalAnimals, GetTotalAnimalStmtType),
		}
